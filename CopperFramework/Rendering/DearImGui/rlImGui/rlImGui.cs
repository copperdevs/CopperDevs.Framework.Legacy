...
public static class rlImGui
{
    ...
    public static void EndInitImGui()
    {
        ...
        unsafe
        {
            ...
            fixed (ushort* range = &IconRanges[0])
            {
                ...
                icons_config->GlyphRanges = (ushort*)FontAwesome.IconFontRanges.ToPointer();

                byte[] fontDataBuffer = Convert.FromBase64String(FontAwesome.IconFontData);

                fixed (byte* buffer = fontDataBuffer)
                {
                    var fontPtr = ImGui.GetIO().Fonts.AddFontFromMemoryTTF(new IntPtr(buffer), fontDataBuffer.Length, 11, icons_config, FontAwesome.IconFontRanges);
                }
            }
    ...
    public static void ImageFit(Texture2D image, bool center = true)
    {
